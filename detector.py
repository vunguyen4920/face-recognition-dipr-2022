...
def main_app(personName):
        
    face_cascade = cv2.CascadeClassifier('./data/haarcascade_frontalface_alt.xml')
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    recognizer.read(f"./data/classifiers/{personName}_classifier.xml")
    ...
    while True:
        ...
        if cv2.waitKey(20) & 0xFF == ord('q'):
            ...
            if pred > 0 : 
                dim =(124,124)
                img = cv2.imread(f".\\data\\{personName}\\{pred}{personName}.jpg", cv2.IMREAD_UNCHANGED)
                resized = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
                cv2.imwrite(f".\\data\\{personName}\\50{personName}.jpg", resized)
                Image1 = Image.open(f".\\2.png") 
                    
                # make a copy the image so that the  
                # original image does not get affected 
                Image1copy = Image1.copy() 
                Image2 = Image.open(f".\\data\\{personName}\\50{personName}.jpg") 
